#READ.ME TURN ON DEVELOPER MODE IN DISCORD. Please fil in the correct information in the "1234" (it goes to line 182) in the beginning and further on! Watch out with Changing Commands⚠️!
# ELTRIX.py
# This bot is under a MIT license when used in a server you must credit the orginal creator Example Via the description of the bot
...
TICKET_CATEGORIES = {
    ...
    "report": {
        ...
        "staff_roles": [1234, 1234] # please fol in staff roles
    },
    "question": {
...
WELCOME_CHANNEL_ID = 1234 # Channel ID for welcome messages (Set to your channel ID)
...
def load_json(filename: str) -> dict:
    ...
    if os.path.exists(filename):
        try:
            ...
            print(f"⚠️ Fout: {filename} is beschadigd of leeg. Er wordt een nieuwe aangemaakt.")
            return {}
    return {}
...
#README these are the last couple of things you need to fill in "Role Id 1234" <-- remove Role ID 1234 and ad your 1234 Id Initialize role_menu_data if the file is empty/new
if not role_menu_data:
    role_menu_data = {
        "platforms": {
            ...
            "max_values": 3,
            "roles": {
                "Role Id 1234": {"label": "Playstation", "emoji": "🎮"},
                "Role Id 1234": {"label": "Xbox", "emoji": "🎮"},
                "Role Id 1234": {"label": "PC", "emoji": "🖥️"},
            }
        },
        "genres": {
            ...
            "max_values": 2,
            "roles": {
                "Role Id 1234": {"label": "Roleplay", "emoji": "🎭"},
                "Role Id 1234": {"label": "Action", "emoji": "💥"},
            }
        },
        "games": {
            ...
            "max_values": 2,
            "roles": {
                "Role Id 1234": {"label": "Minecraft", "emoji": "🧱"},
                "Role Id 1234": {"label": "War Thunder", "emoji": "✈️"},
            }
        }
    }
    save_json(ROLE_MENU_FILE, role_menu_data)
#GREAT NEWS THAT WAS ALL YOU NEEDED TO FILL IN 
# --- Helper Functions ---   #WARNING DONT CHANGE ANYTHING ANYMORE BECAUSE THE BOT WIL BREAK UNTIL YOU KNOW WHAT YOURE DOING (MIT LICENSE RESTRICTS SOME COMMANDS)
...
def is_staff():
    """Decorator to check if the user is the owner or has a staff role."""
    def predicate(interaction: discord.Interaction) -> bool:
        return check_if_staff(interaction)
...
# Because of the MIT LICENSE your not allowed to Change this! 
...
async def about(interaction: discord.Interaction):
    ...
    embed.add_field(name="Version", value="3.1.0", inline=True)      #JUST THE VERSION
    embed.add_field(name="Creator", value=f"<@{OWNER_ID}>", inline=True)          # THE CREATOR OF THE SERVER ID
    embed.add_field(name="Discord-Bot Creator", value="<@917498510335672370>", inline=True)     
    embed.add_field(name="Support Server", value="[Join here!](https://discord.gg/kEj6UDdQvb)", inline=False)   
    await interaction.response.send_message(embed=embed)

# --- Background Tasks ---
async def handle_timer_completion(timer_id, data):
    """Handles the completion of a timer (giveaway or reminder)."""
    if data["type"] == "reminder":
        try:
            user = bot.get_user(data["user_id"]) or await bot.fetch_user(data["user_id"])
            if user:
                embed = discord.Embed(title="⏰ Reminder!", description=data["reminder"], color=discord.Color.yellow())
                await user.send(embed=embed)
            else:
                channel = bot.get_channel(data["channel_id"])
                if channel:
                    await channel.send(f"⏰ Reminder for <@{data['user_id']}>: {data['reminder']}")
        except Exception as e:
            print(f"Error sending reminder: {e}")

    elif data["type"] == "giveaway":
        try:
            channel = bot.get_channel(data["channel_id"])
            if channel:
                try:
                    ...
                    if not users:
                        await channel.send(f"The giveaway for **{data['prize']}** has ended. No one entered! 😕")
                    else:
                        chosen_winners = random.sample(users, k=min(data["winners"], len(users)))
                        winner_mentions = ", ".join(w.mention for w in chosen_winners)
                        await channel.send(f"Congratulations {winner_mentions}! You won the **{data['prize']}**! 🎉")
                except discord.NotFound:
                    await channel.send(f"The giveaway message for **{data['prize']}** was deleted, so no winner could be picked.")
        except Exception as e:
            print(f"Error handling giveaway: {e}")

    if timer_id in timers_data:
        del timers_data[timer_id]
        save_json(TIMERS_FILE, timers_data)

async def start_timer(timer_id, data):
    """Waits for the timer to finish and then handles it."""
    now = datetime.datetime.now(datetime.timezone.utc).timestamp()
    remaining = data["end_time"] - now
    if remaining > 0:
        await asyncio.sleep(remaining)
    await handle_timer_completion(timer_id, data)

@tasks.loop(minutes=5)
async def auto_save():
    """Periodically saves data to JSON files to reduce IO operations."""
    save_json(LEVELING_FILE, leveling_data)
...
async def change_status():
    """Automatically changes the bot's status every 5 minutes."""
    statuses = [
        discord.Game(name="with commands"),
        discord.Activity(type=discord.ActivityType.watching, name="Looking over the server, eltrix-dot.github.io"),
        discord.Activity(type=discord.ActivityType.listening, name="your feedback"),
        discord.Game(name="ELTRIX v3.1")
    ]
...
class TicketCreationModal(discord.ui.Modal):
    def __init__(self, category_key: str, category_details: dict):
        ...
        self.relevant_user_id = discord.ui.TextInput(
            ...
            placeholder="If your ticket is about a specific user, please provide their ID Eltrix-Bot.",
            style=discord.TextStyle.short,
            required=False,
            max_length=20,
        )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not discord_category:
            await interaction.followup.send("❌ Ticket system is not configured correctly (Category not found). Please go to the support server for help: [Join here!](https://discord.gg/kEj6UDdQvb)", ephemeral=True)
            return
...
class RoleSelect(discord.ui.Select):
    def __init__(self, category_key: str, category_config: dict):
        ...
        self.category_roles = []
        valid_items = []
        for k, v in category_config.get("roles", {}).items():
            try:
                self.category_roles.append(int(k))
        ...
        options = [
            ...
            ) for role_id, details in valid_items
        ]

        if not options:
            options.append(discord.SelectOption(label="No valid roles configured", value="0", description="Check configuration"))

        super().__init__(
            placeholder=category_config["placeholder"],
            min_values=0,
            max_values=min(category_config["max_values"], len(options)),
            options=options,
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.followup.send("✅ Your roles have been updated! - Eltrix-Bot", ephemeral=True)
...
async def commands(interaction: discord.Interaction):
    ...
    def get_command_details(cmd_names):
        details = []
    ...
    # Staff Commands List
    staff_cmds = [
        "altcheck", "announce", "audit", "ban", "botstats", "clearwarnings", 
        "giveaway", "history", "kick", "lock", "lockdown", "massmove", "mute", 
        "note", "presetmessage", "purge", "removelevelrole", "rolemenu", 
        "setlevelrole", "setupuserroles", "setupverification", "setxp", 
        "slowmode", "softban", "ticketadd", "ticketclaim", "ticketdelete", 
        "ticketremove", "ticketrename", "ticketsetup", "unlock", "unlockdown", 
        "unmute", "warn", "warnings", "wipebotmessages"
    ]

    # Public Commands List
    public_cmds = [
        "8ball", "about", "accountage", "afk", "avatar", "balance", "bank", "banner",
        "boosters", "botinvite", "calendar", "coinflip", "commands", "countdown", 
        "dadjoke", "daily", "dare", "emojistats", "fact", "feedback", "gamble", 
        "give", "hug", "invest", "leaderboard", "levelboard", "levelroles", 
        "membercount", "meme", "messagestats", "mock", "pat", "ping", "poll", 
        "rank", "remindme", "rep", "repleaderboard", "roast", "roleinfo", 
        "serverage", "serverinfo", "ship", "slap", "slots", "spinbottle", 
        "stopwatch", "suggest", "tag", "timer", "topchatters", "truth", 
        "uptime", "urban", "userinfo"
    ]

    embed = discord.Embed(
        title="📜 Bot Command List",
        description=f"Here are all the available commands for **{interaction.guild.name}**.",
        color=discord.Color.blurple(),
        timestamp=datetime.datetime.now(datetime.timezone.utc)
    )

    # Add Staff Commands (Only if user is staff)
    if check_if_staff(interaction):
        staff_details = get_command_details(staff_cmds)
        if staff_details:
            staff_text = "\n".join(staff_details)
            for i, chunk in enumerate(chunk_string(staff_text, 1024)):
                name = "👮 Staff Commands" if i == 0 else "👮 Staff Commands (cont.)"
                embed.add_field(name=name, value=chunk, inline=False)

    # Add Public Commands
    public_details = get_command_details(public_cmds)
    if public_details:
        public_text = "\n".join(public_details)
        for i, chunk in enumerate(chunk_string(public_text, 1024)):
            name = "🌍 Public Commands" if i == 0 else "🌍 Public Commands (cont.)"
            embed.add_field(name=name, value=chunk, inline=False)
...
async def ticketremove(interaction: discord.Interaction, member: discord.Member):
    ...
    if channel_overwrites.read_messages is True and not any(role.id in STAFF_ROLE_IDS for role in member.roles):
...
async def giveaway(interaction: discord.Interaction, prize: str, minutes: int, winners: int = 1): # This command should remain staff-only as it's a moderation/event tool
    ...
    timers_data[timer_id] = timer_data
    save_json(TIMERS_FILE, timers_data)
    bot.loop.create_task(start_timer(timer_id, timer_data))
...
@tree.command(name="servericon", description="Displays the server's icon. [Everyone]", guild=GUILD)
async def servericon(interaction: discord.Interaction):
    """Displays the server's icon in high quality."""
    if not interaction.guild.icon:
        await interaction.response.send_message("❌ This server has no icon.", ephemeral=True)
        return
    
    embed = discord.Embed(title=f"Icon of {interaction.guild.name}", color=discord.Color.blue())
    embed.set_image(url=interaction.guild.icon.url)
    await interaction.response.send_message(embed=embed)

@tree.command(name="banner", description="Displays a user's banner. [Everyone]", guild=GUILD)
...
async def banner(interaction: discord.Interaction, user: Optional[discord.Member] = None):
    ...
    # Need to fetch user to get banner info if not cached
    user_data = await bot.fetch_user(target.id)
    
    if not user_data.banner:
        await interaction.response.send_message(f"❌ {target.display_name} does not have a banner.", ephemeral=True)
        return

    embed = discord.Embed(title=f"Banner of {target.display_name}", color=target.color)
...
async def on_ready():
    ...
    # Resume timers
    for timer_id, data in list(timers_data.items()):
        bot.loop.create_task(start_timer(timer_id, data))
    ...
    if GUILD_ID == 1234:
         print("⚠️ WAARSCHUWING: Je hebt GUILD_ID nog niet ingesteld in de config!")

    if not STAFF_ROLE_IDS or 1234 in STAFF_ROLE_IDS:
         print("⚠️ WAARSCHUWING: Controleer je STAFF_ROLE_IDS. '1234' staat er nog in.")
    ...
    if startup_channel:
        ...
        embed.set_footer(text=f"{bot.user.name} | Version 3.1.0")
        try:
            await startup_channel.send(embed=embed)
        except Exception as e:
            print(f"⚠️ Kon startup bericht niet versturen (netwerkfout): {e}")
    else:
...
async def on_member_join(member: discord.Member):
    ...
    if welcome_channel:
        embed = discord.Embed(
            ...
            description=f"Hello {member.mention}, welcome to the server! We are glad to have you here.",
            color=discord.Color.green(),
            timestamp=datetime.datetime.now(datetime.timezone.utc)
        )
...
async def on_app_command_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    ...
    if isinstance(error, app_commands.CheckFailure):
        ...
        error_embed.description = f"Could not find a valid user/member matching `{error.value}`. Please select a user from the list provided by Discord."

    elif isinstance(error, app_commands.MissingPermissions):
        error_embed.title = "🔒 Missing Permissions"
        error_embed.description = f"You are missing the following permissions: {', '.join(error.missing_permissions)}"

    elif isinstance(error, app_commands.BotMissingPermissions):
        error_embed.title = "🔒 Bot Missing Permissions"
        error_embed.description = f"I am missing the following permissions: {', '.join(error.missing_permissions)}"
...
async def give(interaction: discord.Interaction, user: discord.Member, amount: int):
    ...
    embed = discord.Embed(
        ...
        description=f"✅ You have successfully given **{amount}** coins to {user.mention}!",
        color=discord.Color.green()
    )
    await interaction.response.send_message(embed=embed)
...
async def bank_deposit(interaction: discord.Interaction, amount: str):
    ...
    embed = discord.Embed(
        ...
        description=f"✅ You have deposited **{deposit_amount}** coins into your bank.\n\n**New Bank Balance:** {economy_data[user_id_str]['bank']} coins",
        color=discord.Color.green()
    )
    await interaction.response.send_message(embed=embed, ephemeral=True)
...
async def bank_withdraw(interaction: discord.Interaction, amount: str):
    ...
    embed = discord.Embed(
        ...
        description=f"✅ You have withdrawn **{withdraw_amount}** coins from your bank.\n\n**New Wallet Balance:** {economy_data[user_id_str]['balance']} coins",
        color=discord.Color.green()
    )
    await interaction.response.send_message(embed=embed, ephemeral=True)
...
@is_owner()
async def privatemessage(interaction: discord.Interaction, user: discord.User, message: str, as_embed: bool, color: Optional[app_commands.Choice[str]] = None):
...
@is_owner()
async def privatewarn(interaction: discord.Interaction, user: discord.User, reason: str):
